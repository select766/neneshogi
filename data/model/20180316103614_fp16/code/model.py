...
init_w = {"initialW": initializers.HeNormal(1.0 / np.sqrt(2), np.float16),
          "initial_bias": initializers.Zero(np.float16)}
init_bn = {"dtype": np.float16}


class ResBlock(chainer.Chain):
    def __init__(self, in_ch, out_ch):
        super().__init__(
            conv1=L.Convolution2D(in_ch, out_ch, 3, pad=1, nobias=True, **init_w),
            conv2=L.Convolution2D(out_ch, out_ch, 3, pad=1, nobias=True, **init_w),
            conv3=L.Convolution2D(out_ch, out_ch, 3, pad=1, nobias=True, **init_w),
            bn1=L.BatchNormalization(in_ch, **init_bn),
            bn2=L.BatchNormalization(out_ch, **init_bn),
            bn3=L.BatchNormalization(out_ch, **init_bn),
        )
...
class Model(chainer.Chain):
    ...
    def __init__(self, ch=16, depth=4, weight_move=1.0, weight_value=1.0):
        chains = {}
        W = initializers.HeNormal(1.0, np.float16)
        bias = initializers.Zero(np.float16)
        chains["conv_first"] = L.Convolution2D(None, ch, 5, pad=2, nobias=True, **init_w)
        chains["bn_first"] = L.BatchNormalization(ch, **init_bn)
        chains["res_blocks"] = chainer.ChainList(*[ResBlock(ch, ch) for i in range(depth)])
        chains["move_res_block"] = ResBlock(ch, ch)
        chains["move_conv"] = L.Convolution2D(None, 139, 3, pad=1, **init_w)
        chains["value_res_block"] = ResBlock(ch, ch)
        chains["value_fc"] = L.Linear(None, 1, **init_w)
